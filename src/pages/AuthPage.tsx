...
export default function AuthPage() {
  ...
  async function handleForgot(e: FormEvent) {
    ...
    try {
      ...
      setSuccessMsg(`Password reset email sent to ${studentCode.toLowerCase()}@gmail.com`)
    } catch (err: unknown) {
      setError(friendlyError(err))
    } finally {
      setLoading(false)
        ...
        <div className="bg-white/5 backdrop-blur-xl border border-white/10 rounded-3xl p-8 shadow-2xl">
          ...
          {view === 'signup' && (
            <form onSubmit={handleSignUp} className="space-y-4">
              ...
              <p className="text-xs text-white/30 -mt-1">
                Your password will be set to your student code. You can change it later.
              </p>
          ...
          {view === 'forgot' && (
            <form onSubmit={handleForgot} className="space-y-4">
              ...
              <p className="text-xs text-white/30 -mt-1">
                A reset link will be sent to <span className="text-white/50">{studentCode ? `${studentCode.toLowerCase()}@gmail.com` : 'studentcode@gmail.com'}</span>
              </p>
